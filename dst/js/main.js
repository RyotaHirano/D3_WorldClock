 ...
 *   homepage: https://github.com/mbostock/queue#readme
 *   version: 1.0.7
 * 
...
//裏側の設定
var projectionBak = _D2.default.geo.orthographic().translate([600, 400]).scale(scale).rotate([0, 0, 0]).clipAngle(180); //裏側

var projectionDummy = _D2.default.geo.orthographic().translate([600, 400]).scale(scale).rotate([0, 0, 0]).clipAngle(89);

//表側の設定
var projection = _D2.default.geo.orthographic().translate([600, 400]).scale(scale).rotate([0, 0, 0]).clipAngle(90);

var bakPath = _D2.default.geo.path().projection(projectionBak);
var pathDummy = _D2.default.geo.path().projection(projectionDummy);
...
function createGeo() {
	stage = stage.append('g').attr('transform', 'rotate(23.4, ' + 600 + ', ' + 400 + ')');
	stage2 = stage2.append('g').attr('transform', 'rotate(23.4, ' + 600 + ', ' + 400 + ')');

	(0, _queueAsync2.default)().defer(_D2.default.json, "https://dl.dropboxusercontent.com/u/33487063/geo/earth.geojson").await(geoReady);
}
...
function geoReady(error, world) {
	...
	//裏面(stage1)
	var bakMap = stage.append("svg:path").attr({
		"d": bakPath,
		"fill-opacity": 1,
		"fill": "#EDE9F1",
		"stroke": "none"
	...
	//表面(stage1)
	var map = stage.selectAll('path').data(world.features).enter().append('path').attr({
		"d": function d(_d, i) {
  ...
  "timezone": function(d, i) {
	...
	// click時の面(stage2)
	var mapDummy = stage2.selectAll('path').data(world.features).enter().append('path').attr({
		"d": function d(_d2, i) {
	...
	var rotateEarth = function rotateEarth() {
		...
		bakPath = _D2.default.geo.path().projection(projectionBak);
		pathDummy = _D2.default.geo.path().projection(projectionDummy);
		path = _D2.default.geo.path().projection(projection);
  	...
  	setWorldTime(timezone);
		...
		mapDummy.attr({
			...
			var countryName = _D2.default.select(this).attr('country-name');
			setCountryName(countryName);

			var timezone = _D2.default.select(this).attr('timezone');
			setWorldTime(timezone);
		});
	...
	// zoomの設定
	var zoom = _D2.default.behavior.zoom().on('zoom', function () {
		projection.scale(scale * _D2.default.event.scale);
		map.attr('d', path);
	...
	// drag rollの設定
	var roll = _D2.default.behavior.drag().origin(function () {
		var r = projection.rotate();
		return { x: r[0] / 0.25, y: r[1] * 0.25 * -1 };
	}).on('drag', function () {
		cancelAnimationFrame(rotateAnimation);

		var newX = _D2.default.event.x * 0.25;
		var newY = _D2.default.event.y * 0.25 * -1;
...
function setWorldTime(timezone) {
	...
	_D2.default.select('#worldDate').text(formatYYYYMMDDW);
	...
	_D2.default.select('#worldTime').text(formatSelectCountryTime);
}

init();

},{"D3":1,"queue-async":2}]},{},[3]);
